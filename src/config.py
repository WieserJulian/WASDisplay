...
class Config(object):
    ...
    def load_graphs(self):
            self.g_overview = ox.graph_from_address(self.default['place_depo'], dist=15_000, network_type="all_private", simplify=False,
                                               retain_all=True)
            self.g_drive = ox.graph_from_address(self.default['place_depo'], dist=15_000, network_type="drive", simplify=False,
                                               retain_all=True)
            ox.save_graphml(self.g_overview, "save/graph_overview")
            ox.save_graphml(self.g_drive, "save/graph_drive")
            tags = {'building': True}
            self.gdf_depo = ox.features_from_place(self.default['place_depo'], tags)
            self.depo_way, _ = ox.nearest_nodes(self.g_overview, self.gdf_depo.centroid.x.values, self.gdf_depo.centroid.y.values,
                                           return_dist=True)
