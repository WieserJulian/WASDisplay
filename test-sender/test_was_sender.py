...
class App(customtkinter.CTk):
    ...
    def add_em(self):
        ...
        operation_id.grid(row=2, column=0, padx=5)
        operation_name = customtkinter.CTkEntry(frame, placeholder_text="Stichwort")
        operation_name.grid(row=0, column=0, padx=5)
        caller_name = customtkinter.CTkEntry(frame, placeholder_text="Anrufer name")
        caller_name.grid(row=0, column=1, padx=5)
        caller = customtkinter.CTkEntry(frame, placeholder_text="Anrufer Nummer")
        caller.grid(row=0, column=2, padx=5)
        operation_address = customtkinter.CTkEntry(frame, placeholder_text="Addresse")
        operation_address.grid(row=0, column=3, padx=5)
        info = customtkinter.CTkEntry(frame, placeholder_text="Informationen")
        info.grid(row=0, column=4, padx=5)
        program = customtkinter.CTkEntry(frame, placeholder_text="Programm")
        program.insert(0, "Echt Alarm")
        program.grid(row=0, column=5, padx=5)
        operation_state = customtkinter.CTkOptionMenu(frame, values=['None', 'Alarmiert', 'Ausgerückt', 'Fertig'])
        operation_state.set('None')
        operation_state.grid(row=1, column=0, padx=5)
        send = customtkinter.CTkButton(frame, text="Rebuild", command=lambda:
        self.rebuild_em(operation_id.cget('text'),
                        caller_name.get(),
                        operation_name.get(),
                        caller.get(),
    ...
    def rebuild_em(self, id, name, operation_name, caller, address, info, programm, state):
        emergency = [name, operation_name, caller, address, info, programm, state]
        match state:
            case 'None':
                return
            case 'Alarmiert':
    ...
    def send_fc(self):
        ...
        try:
            self.status_action.configure(
                text="Sending all 2sec: " + ("Leerliste" if self.command == "<pdu></pdu>" else "Alarmliste"))
            self.socket.send(x)
            ...
            for index, em_id in enumerate(self.all_emergency.keys()):
                ...
                self.command += f""">
                                <order index="{index}">
                                        ...
                                        <receive-tad>{time.strftime("%Y.%m.%d %H:%M:%S")}</receive-tad>
                                        <operation-id>{em_id}</operation-id>
                                        <level>1</level>
                                        <name>{em[0]}</name>
                                        <operation-name>{em[1]}</operation-name>
                                        <caller>{em[2]}</caller>
                                        <location>{em[3]}</location>
                                        <info>{em[4]}</info>
                                        <program>{em[5]}</program>
                                        <status>{em[6]}</status>
                                        <watch-out-tad></watch-out-tad>
                                        <finished-tad></finished-tad>
                                        <destination-list count="1">
                                        <destination index="1" id="34101">Rutzenham</destination>
                                        </destination-list>
